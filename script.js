...
form.addEventListener("submit", (e) => {
  ...
  if (!number.value || number.value.length < 16) {
    number.classList.add("invalid");
    numberError.innerText = "Can't be blank";
  } else if (!number.value.match(numberRegExp)) {
    number.classList.add("invalid");
  ...
  if (!month.value || !year.value) {
    ...
    !month.value.match(monthRegExp) ||
    !year.value.match(yearRegExp) ||
    year.value.length < 2
  ) {
    month.classList.add("invalid");
    year.classList.add("invalid");
  ...
  let cvcRegExp = /^(00[0-9]|0[1-9][0-9]|[1-9][0-9][0-9])$/;

  if (!cvcNumber.value) {
    cvcNumber.classList.add("invalid");
    cvcError.innerText = "Can't be blank";
  } else if (!cvcNumber.value.match(cvcRegExp)) {
    cvcNumber.classList.add("invalid");
